...
Active Members are required to pay dues as stated in \ref{Collection of Dues}, attend all House Meetings and E-Board candidate speeches, and attend at least fifteen directorship meetings for each semester in which they are an Active Member.
They are also expected to sign a copy of the Membership Agreement.
\\* \\*
Active Members must participate in at least one major project.
...
Any of these requirements may be waived by an E-Board Vote or an Immediate Relative Majority vote with fifty-percent quorum.
The waiver will last until the end of the semester, or until there is a negative change in the status of the waived requirement(s).
\begin{enumerate}
...
Active Members are evaluated annually through the Membership Evaluation Process.

% ACTIVE MEMBERSHIP EVALUATIONS
\asubsubsection{Membership Evaluation}
The Eval Process occurs once per academic year, where Active Members are evaluated.
The Eval Process takes place during the spring semester to comply with the RIT Housing deadlines.
The Eval Process is responsible for determining individuals who will have the option to continue Active Membership in the following academic year.
...
Any Active Member who has completed all the requirements as defined in \ref{Expectations of Active Members} at the beginning of the Eval Process pass the Eval Process without being voted on.
All Active Members who have not received an exemption from E-Board prior to the start of the Eval Process will be evaluated on a member-by-member basis by Active Members.
The Eval Process shall hold members to the objective requirements defined in \ref{Expectations of Active Members} and determine which members may continue as an Active Member in the following academic year.
...
These requirements must be completed before the Eval Process occurs.
...
Members who pass the Eval Process have the option to participate as an Active Member in the following year.
\\* \\*
If a member fails and has never passed an Eval Process in the past, their membership will be revoked upon completion of the Eval Process.
If the member fails, but has previously passed an Eval Process, they will move to Alumni Membership in bad standing, as defined in \ref{Alumni Membership Selection}, at the end of the Standard Operating Session.
...
If the conditional is approved by the Evals Director, it is then voted on by the attending members as described in \ref{Membership Evaluation Voting}.
Each conditional consists of a set of additional requirements and a deadline to complete them.
Members who are given a conditional have their Eval Process decision deferred and therefore maintain their current membership status until the conditional is resolved.
When the deadline expires, the conditional will be brought before E-Board, who will assess its completeness.
A conditional member who does not meet these additional requirements will have failed the Eval Process.
...
\begin{enumerate}
	\item Candidates must be an RTP.
...
\begin{enumerate}
	...
	\item The term of an officer will be abbreviated due to resignation, impeachment, change in membership status, loss of good academic standing, or receiving a conduct sanction of disciplinary probation or higher.
...
\begin{enumerate}
	...
	\item Obtain greater than or equal to 60\% of required signatures in the Intro Packet, excluding Intro Members, or successfully complete the Intro Eval.
...
The eligible voters for a vote are determined and fixed when the Voting Period begins.
